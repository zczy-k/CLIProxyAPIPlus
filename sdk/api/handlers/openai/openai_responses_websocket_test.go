...
func TestWebsocketUpstreamSupportsIncrementalInputForAliasedModel(t *testing.T) {
	manager := coreauth.NewManager(nil, nil, nil)
	manager.SetConfig(&internalconfig.Config{})
	manager.SetOAuthModelAlias(map[string][]internalconfig.OAuthModelAlias{
		"github-copilot": {{Name: "gemini-3-pro-preview", Alias: "gemini-3.1-pro-co", Fork: true}},
	})
	auth := &coreauth.Auth{
		ID:         "auth-ws-alias",
	...
	registry.GetGlobalRegistry().RegisterClient(auth.ID, auth.Provider, []*registry.ModelInfo{{ID: "gemini-3-pro-preview"}})
	t.Cleanup(func() {
		registry.GetGlobalRegistry().UnregisterClient(auth.ID)
	})
	...
	base := handlers.NewBaseAPIHandlers(&sdkconfig.SDKConfig{}, manager)
	h := NewOpenAIResponsesAPIHandler(base)
	if !h.websocketUpstreamSupportsIncrementalInputForModel("gemini-3.1-pro-co") {
		t.Fatalf("expected websocket-capable upstream for aliased model")
	}
}
